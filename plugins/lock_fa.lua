...
if msg.to.type == 'chat' then
  ...
  if data[tostring(msg.to.id)]['settings']['lock_fa'] then
    if data[tostring(msg.to.id)]['settings']['lock_fa'] == 'yes' then
      if msg.from.id == true then 
        return
      end
      send_large_msg("chat#id".. msg.to.id , "⛔️حروف فارسی مجاز نمیباشد.")
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] kicked (lock_fa was locked) ")
