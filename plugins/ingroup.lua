...
local function show_group_settingsmod(msg, data, target)
  ...
  local text = "⚙ تنظیمات گروه :\nℹ قفل اسم گروه : "..settings.lock_name.."\nℹ قفل عکس گروه : "..settings.lock_photo.."\nℹ قفل ارسال تگ : "..settings.lock_tag.."\nℹ قفل اددکردن ممبر  : "..settings.lock_member.."\nℹ قفل انگلیسی حرف زدن : "..settings.lock_eng.."\nℹ قفل خروج از گروه=بن : "..settings.leave_ban.."\nℹ قفل عربی حرف زدن : "..settings.lock_arabic.."\nℹ قفل فحش دادن : "..settings.lock_fosh.."\nℹ قفل شماره  : "..settings.lock_share.."\nℹ قفل چت کردن : "..settings.lock_chat.."\nℹ قفل ارسال گیف : "..settings.lock_gif.."\nℹ قفل ارسال اموجی : "..settings.lock_emogi.."\nℹ قفل گذاشتن ویس،ویدیو : "..settings.lock_media.."\nℹ قفل فحش انگلیسی : "..settings.lock_badw.."\nℹ قفل تبلیغات و لینک : "..settings.lock_link.."\nℹ قفل ورود اعضای جدید با لینک : "..settings.lock_join.."\nℹFlood Sensitivity : "..NUM_MSG_MAX.."\nℹ Bot Protection : "..bots_protection--"\nPublic: "..public
  return text
end
...
local function lock_group_fa(msg, data, target)
  ...
  local group_fa_lock = data[tostring(target)]['settings']['lock_fa']
  if group_fa_lock == 'yes' then
    return 'english is already locked!'
  else
    data[tostring(target)]['settings']['lock_fa'] = 'yes'
    save_data(_config.moderation.data, data)
...
local function unlock_group_fa(msg, data, target)
  ...
  if group_fa_lock == 'no' then
    ...
    data[tostring(target)]['settings']['lock_fa'] = 'no'
    save_data(_config.moderation.data, data)
...
local function lock_group_emogi(msg, data, target)
  ...
  local group_emogi_lock = data[tostring(target)]['settings']['lock_emogi']
  if group_emogi_lock == 'yes' then
    return 'emogi is already locked!'
  else
    data[tostring(target)]['settings']['lock_emogi'] = 'yes'
    save_data(_config.moderation.data, data)
...
local function unlock_group_emogi(msg, data, target)
  ...
  local group_emogi_lock = data[tostring(target)]['settings']['lock_emogi']
  if group_emogi_lock == 'no' then
    return 'emogi is already unlocked!'
  else
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[2] == 'leave' then
      ...
      if matches[2] == 'adds' then
        ...
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked fa ")
    ...
    if matches[2] == 'leave' then
      ...
      if matches[2] == 'chat' then
        ...
        return unlock_group_eng(msg, data, target)
          ...
          if matches[2] == 'fa' then
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked fa ")
        return unlock_group_fa(msg, data, target)
      end
          if matches[2] == 'emogi' then
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked emogi ")
