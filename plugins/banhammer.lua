...
--vardump(extra)
      ...
      elseif get_cmd == 'sikban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
          return send_large_msg(receiver, "You can't ban mods/owner/admins")
        end
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] بن شد')
        return ban_user(member_id, chat_id)
      elseif get_cmd == 'sikun' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] انبن شد')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'کاربر '..user_id..'انبن شد'
      elseif get_cmd == 'sikall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از کل گروه ها بن شد')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'sikunban' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از بن کل گروه ها درامد')
 ...
 if matches[1]:lower() == 'id' then
    ...
    if type(msg.reply_id) ~= "nil" then
      local name = user_print_name(msg.from)
        ...
        id = get_message(msg.reply_id,get_message_callback_id, true)
  ...
  if matches[1]:lower() == 'sikban' then-- /ban 
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin(msg) then
        local msgr = get_message(msg.reply_id,ban_by_reply_admins, true)
      else
        msgr = get_message(msg.reply_id,ban_by_reply, true)
      end
    end
      ...
      if string.match(matches[2], '^%d+$') then
        ...
        if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
          	return " mods/owner/admins فقط اینا میتونن"
        end
  ...
  if matches[1]:lower() == 'sikun' then -- /unban 
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      local msgr = get_message(msg.reply_id,unban_by_reply, true)
    end
      local user_id = matches[2]
      local chat_id = msg.to.id
...
if matches[1]:lower() == 'sik' then
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin(msg) then
        local msgr = get_message(msg.reply_id,Kick_by_reply_admins, true)
      else
        msgr = get_message(msg.reply_id,Kick_by_reply, true)
      end
    end
	...
	if string.match(matches[2], '^%d+$') then
		...
		if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
			return " mods/owner/admins اینا میتونن"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
  ...
  if matches[1]:lower() == 'sikall' then -- Global ban
    if type(msg.reply_id) ~="nil" and is_admin(msg) then
      return get_message(msg.reply_id,banall_by_reply, true)
    end
    local user_id = matches[2]
    local chat_id = msg.to.id
      local targetuser = matches[2]
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'کاربر ['..user_id..' ] از کل گروه ها بن شد'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'sikall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
        	...
        	return 'کاربر ['..user_id..' ] از بن کل گروه ها در امد'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'sikunban',
